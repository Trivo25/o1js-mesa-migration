...
console.log('\n=== Test: VK upgrade via signature (expecting success — proofOrSignature) ===');
await fetchAccount({ publicKey: sender });
...
console.log('VK change via signature succeeded (proofOrSignature allows it).');

console.log('\n=== Test: VK upgrade via proof (expecting success — proofOrSignature) ===');
await fetchAccount({ publicKey: zkAppAddress });
await fetchAccount({ publicKey: sender });

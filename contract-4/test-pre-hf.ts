import { AccountUpdate, Bool, Field, Mina, fetchAccount } from 'o1js';
...
console.log('\nCompiling AccessProofZkApp...');
await AccessProofZkApp.compile();
...
console.log('\n=== Test: editState via signature blocked by access: proof (expecting failure) ===');
await fetchAccount({ publicKey: sender });

try {
  const txAccess = await Mina.transaction({ sender, fee: transactionFee }, async () => {
    const accountUpdate = AccountUpdate.createSigned(zkAppAddress);
    accountUpdate.body.update.appState[0] = { isSome: Bool(true), value: Field(999) };
  });
  await sendAndWait(txAccess, [senderKey, zkAppKey]);
  throw new Error('State change via signature succeeded unexpectedly!');
} catch (error: any) {
  if (error.message.includes('succeeded unexpectedly')) throw error;
  console.log(`Expected failure: ${error.message}`);
  console.log(
    'State edit via signature correctly rejected (access: proof blocks non-proof interactions).'
  );
}
...
try {
  ...
  console.log(`Expected failure: ${error.message}`);
  console.log(
    'VK change via signature correctly rejected (impossibleDuringCurrentVersion + access: proof).'
  );
}

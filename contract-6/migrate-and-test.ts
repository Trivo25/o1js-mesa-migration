...
console.log('\nCompiling FullyLockedZkApp with new o1js...');
const { verificationKey: newVK } = await FullyLockedZkApp.compile();
...
const extraAfter = zkApp.extra.get();
console.log(`Counter after: ${counterAfter}`);
console.log(`Marker after: ${markerAfter}`);
console.log(`Extra after: ${extraAfter}`);

counterAfter.assertEquals(counterBefore.add(1));
markerAfter.assertEquals(Field(99));
extraAfter.assertEquals(Field(123));
console.log('Counter incremented, marker and extra preserved.');

console.log('\n=== Migration Summary ===');
console.log(`  Counter before migration: ${counterBefore}`);
console.log(`  Marker (should be 99): ${markerAfter}`);
console.log(`  Extra (should be 123): ${extraAfter}`);
console.log(`  Pre-upgrade tx (new proof, old VK): rejected as expected`);
console.log(`  VK upgrade via signature (all fallbacks active): success`);

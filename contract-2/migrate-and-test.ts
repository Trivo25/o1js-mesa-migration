...
import { LockedVkZkApp } from './contract.js';
...
console.log('\nCompiling LockedVkZkApp with new o1js...');
const { verificationKey: newVK } = await LockedVkZkApp.compile();
...
const zkApp = new LockedVkZkApp(zkAppAddress);
...
console.log(`  VK upgrade via signature (impossible -> fallback): success`);
